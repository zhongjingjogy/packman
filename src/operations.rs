...
impl PackageManager {
    ...
    pub async fn list_packages(&self) -> Result<Vec<models::Package>, Box<dyn Error + Send + Sync>> {
    ...
    pub async fn push_package(&self, package_path: &Path) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        // 检查包是否已存在以及版本冲突
        match self.check_package_conflict(&metadata.name, &metadata.version).await {
    ...
    pub async fn check_package_conflict(&self, package_name: &str, version: &str) 
        ...
        // 过滤出与给定包名相同的包
        let same_name_packages: Vec<&models::Package> = packages
            .iter()
        ...
        for pkg in &same_name_packages {
            if pkg.version == version {
                // 检查包是否被锁定
                if pkg.is_locked {
                    return Err(format!("Package {}@{} is locked and cannot be modified. Reason: {}", 
                        package_name, version, pkg.lock_reason.as_deref().unwrap_or("Unknown")).into());
                }
        ...
        if !higher_versions.is_empty() {
            ...
            return Ok(PackageConflictStatus::HigherVersionExists(highest_version.to_string()));
        }
    ...
    // 强制推送包，忽略冲突
    pub async fn force_push_package(&self, package_path: &Path) -> Result<(), Box<dyn Error + Send + Sync>> {
        // Validate package path exists
        if !package_path.exists() {
            return Err("Package path does not exist".into());
        }
    ...
    pub async fn test_connection(&self) -> Result<(bool, String), Box<dyn Error + Send + Sync>> {
        ...
        // 检查状态码
        if !response.status().is_success() {
            return Ok((false, format!("存储服务返回错误状态码: {}", response.status())));
        }
        ...
        match from_str::<ListObjectsResponse>(&content) {
            Ok(_) => Ok((true, format!("成功连接到存储服务，bucket '{}' 可用", self.bucket.name()))),
            Err(e) => Ok((false, format!("无法解析响应内容，bucket 可能不存在: {}", e))),
        }
    }

    // 锁定特定版本的包，防止被修改
    pub async fn lock_package(&self, package_name: &str, version: &str, reason: &str, user: &str) 
        -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        // 检查包是否存在
        let packages = self.list_packages().await?;
        let found = packages.iter().any(|p| p.name == package_name && p.version == version);
        ...
        // 检查包是否已经被锁定
        if metadata.locked_packages.iter().any(|lp| lp.name == package_name && lp.version == version) {
    ...
    // 解锁特定版本的包
    pub async fn unlock_package(&self, package_name: &str, version: &str) 
        -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        // 查找锁定的包索引
        let index = metadata.locked_packages.iter().position(|lp| 
            lp.name == package_name && lp.version == version
        );
    ...
    // 备份特定版本的包
    pub async fn backup_package(&self, package_name: &str, version: &str, reason: &str) 
        -> Result<(), Box<dyn Error + Send + Sync>> {
        
        // 检查包是否存在
        let packages = self.list_packages().await?;
        let package = packages.iter().find(|p| p.name == package_name && p.version == version);
        
        let package = match package {
            Some(pkg) => pkg,
            None => return Err(format!("Package {}@{} does not exist", package_name, version).into()),
        };
        ...
        let backup_name = format!("{}-{}-backup-{}.zip", 
            package_name, version, now.timestamp());
        ...
        let response = self.client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(format!("Failed to download object for backup: {}", response.status()).into());
        }
    ...
    // 从备份恢复特定版本的包
    pub async fn restore_package_from_backup(&self, package_name: &str, version: &str, timestamp: Option<&str>) 
        -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        // 查找备份
        let mut filtered_backups: Vec<&models::PackageBackup> = metadata.backups.iter()
            .filter(|b| {
                let parts: Vec<&str> = b.original_path.split('.').next()
                    .unwrap_or("")
                    .split('-')
                    .collect();
        ...
        if filtered_backups.is_empty() {
            return Err(format!("No backups found for package {}@{}", package_name, version).into());
        }
        
        // 如果指定了时间戳，找到特定备份
        let backup = if let Some(ts) = timestamp {
            filtered_backups.iter().find(|b| b.timestamp.starts_with(ts))
                .ok_or_else(|| format!("No backup found with timestamp {}", ts))?
        } else {
            // 否则使用最新的备份
            filtered_backups.sort_by(|a, b| b.timestamp.cmp(&a.timestamp));
            filtered_backups.first()
    ...
    // 获取注册表元数据
    async fn get_registry_metadata(&self) -> Result<models::RegistryMetadata, Box<dyn Error + Send + Sync>> {
        // 元数据文件名
    ...
    // 保存注册表元数据
    async fn save_registry_metadata(&self, metadata: &models::RegistryMetadata) 
        -> Result<(), Box<dyn Error + Send + Sync>> {
